...
async function callTool(name: string, args: any, callSid: string): Promise<any> {
  ...
  if (name === 'book_appointment') {
    const res = await fetch(`${BASE_URL}/api/book`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ...args, call_sid: callSid }),
    });
    return await res.json();
  }
...
export async function runTurn(callSid: string, userText: string): Promise<TurnResult> {
  let conversation = await getConversation(callSid);

  // First turn — seed system prompt
  ...
  while (safetyCounter++ < 6) {
    ...
    if (msg.tool_calls && msg.tool_calls.length > 0) {
      for (const tc of msg.tool_calls) {
        ...
        const t1 = Date.now();
        const result = await callTool(tc.function.name, args, callSid);
        console.log(`[${callSid}] step=tool_done name=${tc.function.name} ms=${Date.now() - t1} result=${JSON.stringify(result)}`);
      ...
      continue; // loop back, let model respond to tool results
    }
  ...
  await saveConversation(callSid, conversation);
  return {
    reply: "I'm having trouble processing that. Let me have someone call you back. Goodbye.",
    shouldHangup: true,
  };
}
